...
<body>
	<div id="main" class="wrapper mainAll">
		...
		<div id="tabs-1" class="container">
			...
			<form:form id="reservationClaimDataForm"
				name="reservationClaimDataForm"
				action="${pageContext.request.contextPath}/myReservationExportClaim">
				<c:choose>
					<c:when test="${tDMClaimSearchResultListDTOs ne null}">
							...
							<table class="table outputtable"
								...
								<thead>
									<tr>
										...
										<th bgcolor="#0328FF" scope="col" class="whitefont"><spring:message
												code="label.claim.patientGender" /></th>
										<th bgcolor="#0328FF" scope="col" class="whitefont"><spring:message
								...
								<tbody>
									<c:forEach items="${tDMClaimSearchResultListDTOs}"
										...
										<tr>
											<td align="center"><a
												href="${pageContext.request.servletContext.contextPath}/unreserveClaim?id=${tDMClaimSearchResultListDTOs.claimId}"><spring:message
						...
						<ul class="grdPagination">
							<%
											...
											if (currentPage != 1) {
			   							%>
							<li><a href="myReservationClaim?page=<%= 1 %>">First</a>
							<div>First</div></li>
							<li><a href="myReservationClaim?page=<%= currentPage-1 %>">&lt;
									Prev</a>
							<div>&lt; Prev</div> <%
			   								} else {
			   								 	if(noOfPages > 1) {
			   							%>
							<li class="disable"><a
								href="myReservationClaim?page=<%= 1 %>">First</a>
							<div>First</div></li>
							<li class="disable"><a
								href="myReservationClaim?page=<%= currentPage-1 %>">&lt;
									Prev</a>
							<div>&lt; Prev</div> <%
			   								 	}
			   								}
							...
							<%
			    						...
			    						%>
							<li><a href="myReservationClaim?page=<%= i %>"
								id="employeeLink"><%= i %></a>
							<div><%= i %></div></li>
							<%
			    									}
										...
										%>
							<li><a href="myReservationClaim?page=<%= currentPage+1 %>">Next
									&gt;</a>
							<div>Next &gt;</div></li>
							<li><a href="myReservationClaim?page=<%= noOfPages %>">Last</a>
							<div>Last</div></li>
							<%
											} else {
											    if(noOfPages > 1) {
										%>
							<li class="disable"><a
								href="myReservationClaim?page=<%= currentPage+1 %>">Next
									&gt;</a>
							<div>Next &gt;</div></li>
							<li class="disable"><a
								href="myReservationClaim?page=<%= noOfPages %>">Last</a>
							<div>Last</div></li>
							<%
											    }
											}
