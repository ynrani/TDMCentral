...
<body>
	<div id="main" class="wrapper mainAll">
		...
		<div id="tabs-1" class="container">
			...
			<form:form id="reservationProviderDataForm"
				name="reservationProviderDataForm"
				action="${pageContext.request.contextPath}/myReservationExportProv">
				<c:choose>
					<c:when test="${fTDProvSearchResultListDTOList ne null}">
						...
						<div id="tabs-1" class="container scrollingX">
							<table class="table outputtable"
								...
								<tbody>
									<c:forEach items="${fTDProvSearchResultListDTOList}"
										var="fTDProvSearchResultListDTOList" varStatus="status">
										<tr>
											<td><a
												href="${pageContext.request.servletContext.contextPath}/unreserveProv?id=${fTDProvSearchResultListDTOList.providerId}"><spring:message
						...
						<ul class="grdPagination">
							<%
											...
											if (currentPage != 1) {
			   							%>
							<li><a href="myReservationProv?page=<%= 1 %>">First</a>
							<div>First</div></li>
							<li><a href="myReservationProv?page=<%= currentPage-1 %>">&lt;
									Prev</a>
							<div>&lt; Prev</div> <%
			   								} else {
			   								 	if(noOfPages > 1) {
			   							%>
							<li class="disable"><a
								href="myReservationProv?page=<%= 1 %>">First</a>
							<div>First</div></li>
							<li class="disable"><a
								href="myReservationProv?page=<%= currentPage-1 %>">&lt; Prev</a>
							<div>&lt; Prev</div> <%
			   								 	}
			   								}
											if(noOfPages > 1) {
							...
							<%
			    						...
			    						%>
							<li><a href="myReservationProv?page=<%= i %>"
								id="employeeLink"><%= i %></a>
							<div><%= i %></div></li>
							<%
			    									}
										...
										%>
							<li><a href="myReservationProv?page=<%= currentPage+1 %>">Next
									&gt;</a>
							<div>Next &gt;</div></li>
							<li><a href="myReservationProv?page=<%= noOfPages %>">Last</a>
							<div>Last</div></li>
							<%
											} else {
											    if(noOfPages > 1) {
										%>
							<li class="disable"><a
								href="myReservationProv?page=<%= currentPage+1 %>">Next &gt;</a>
							<div>Next &gt;</div></li>
							<li class="disable"><a
								href="myReservationProv?page=<%= noOfPages %>">Last</a>
							<div>Last</div></li>
							<%
											    }
											}
