...
<body>
	<div id="main" class="wrapper mainAll">
		...
		<div id="tabs-1" class="container">
			...
			<form:form id="reservationClaimSearchDataForm"
				name="reservationClaimSearchDataForm"
				action="${pageContext.request.contextPath}/myReservationExportClaimSearch">
				<c:choose>
					<c:when test="${tdmClaimSearchResultDTOs ne null}">
						...
						<div id="tabs-1" class="container scrollingX">
							<table class="table outputtable"
								...
								<tbody>
									<c:forEach items="${tdmClaimSearchResultDTOs}"
										var="tdmClaimSearchResultDTOs" varStatus="status">
										<tr>
											<td><a
												href="${pageContext.request.servletContext.contextPath}/unreserveClaimSearch?id=${tdmClaimSearchResultDTOs.claimNo}"><spring:message
						...
						<ul class="grdPagination">
							<%
											...
											if (currentPage != 1) {
			   							%>
							<li><a href="myReservationClaimSearch?page=<%= 1 %>">First</a>
							<div>First</div></li>
							<li><a
								href="myReservationClaimSearch?page=<%= currentPage-1 %>">&lt;
									Prev</a>
							<div>&lt; Prev</div> <%
			   								} else {
			   								 	if(noOfPages > 1) {
			   							%>
							<li class="disable"><a
								href="myReservationClaimSearch?page=<%= 1 %>">First</a>
							<div>First</div></li>
							<li class="disable"><a
								href="myReservationClaimSearch?page=<%= currentPage-1 %>">&lt;
									Prev</a>
							...
							<%
			    						...
			    						%>
							<li><a href="myReservationClaimSearch?page=<%= i %>"
								id="employeeLink"><%= i %></a>
							<div><%= i %></div></li>
							<%
			    									}
										...
										%>
							<li><a
								href="myReservationClaimSearch?page=<%= currentPage+1 %>">Next
									&gt;</a>
							<div>Next &gt;</div></li>
							<li><a href="myReservationClaimSearch?page=<%= noOfPages %>">Last</a>
							<div>Last</div></li>
							<%
											} else {
											    if(noOfPages > 1) {
							...
							<div>Next &gt;</div></li>
							<li class="disable"><a
								href="myReservationClaimSearch?page=<%= noOfPages %>">Last</a>
							<div>Last</div></li>
							<%
											    }
											}
