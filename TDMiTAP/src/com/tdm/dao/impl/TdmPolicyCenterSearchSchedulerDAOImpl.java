/*
 * Object Name : TdmPolicyCenterSearchSchedulerDAOImpl.java
 * Modification Block
 * ------------------------------------------------------------------
 * S.No.	Name 			Date			Bug_Fix_No			Desc
 * ------------------------------------------------------------------
 * 	1.	  vkrish14		9:26:08 PM				Created
 * ------------------------------------------------------------------
 ...
 *
 */
public class TdmPolicyCenterSearchSchedulerDAOImpl{
	...
	*/
	...
	public List<TdmReservationDO> getReservedRecords(String userName, EntityManager managerUser)
		...
		try {
			...
			throw new DAOException("DAO exception raised");
		} catch (IllegalArgumentException illegalArgEx) {
	...
	public List<TdmReservationDO> getReservedRecords(String searchType, String userId, int offSet,
			int recordsperpage, boolean pageNationOnOffFlag,List<String> policys, EntityManager managerUser)
			throws DAOException{
		try {
			long startTime = System.currentTimeMillis();
			StringBuffer strPolices = new StringBuffer();
			int icount = 0;
			for (String policyNumber : policys) {
				icount++;
				strPolices.append("'" + policyNumber + "'");
					if(icount < policys.size())
						strPolices.append(",");
			}
			@SuppressWarnings("unchecked")
			List<TdmReservationDO> list = managerUser.createQuery(
					" FROM TdmReservationDO p where p.reserveDataType='PC' AND p.userId='" + userId + "' AND p.reserveDataId IN("+strPolices.toString()+")").getResultList();
			long stopTime = System.currentTimeMillis();
			long elapsedTime = stopTime - startTime;
			if ((elapsedTime / 60000) > 1) {
				throw new DAOException(
	...
	public List<PctlCountryDO> getCountryCodes(EntityManager managerPolicySearch){
	...
	public List<PctlStateDO> getStateCodes(EntityManager managerPolicySearch){
