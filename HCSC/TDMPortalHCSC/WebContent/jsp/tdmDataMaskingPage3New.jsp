     	 		...
     	 		<c:when test="${(readOnly eq null) && (disabled eq null)}">
     	 			<td class="noWrapTxt algnRght" >
           			<c:choose>
						...
						<c:otherwise>
							<a href="javascript:;" class="delRowFromDB_DMASK sprite icon-delete" id="tdgDataMaskingNoOfAppsDTOs[${status.index}].id">Delete</a>
						</c:otherwise>
  					</c:choose>
  					</td>
         		</c:when> 
...
$(document).ready(function() {
	...
	$("#readBack").click(function(){
	 ...
	 $("#cancel").click(function(){
		confirmationForCancel('Cancel','Are you sure you want to cancel ?',400,'YesNo','Yes','No','${tdgDataMaskingDTO.id}'); 
 	  });
