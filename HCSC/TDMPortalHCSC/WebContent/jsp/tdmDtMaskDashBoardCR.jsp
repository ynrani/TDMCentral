                  ...
                  <td class="lable-title" align="left" valign="middle"> Request has been failed to 'Canceled'.</td>
								...
								<tbody>
									<c:forEach items="${tdgDtMaskRequestListDTOs}"
										...
										<tr>											
											...
											onClick="getRequestDtls('${tdgDtMaskRequestListDTOs.id}','${tdgDtMaskRequestListDTOs.status}')"align="left">${tdgDtMaskRequestListDTOs.id}</td>
						...
						<ul class="grdPagination">
							<%
											...
											if (currentPage != 1) {
			   							%>
							<li><a href="tdmDtMaskDashBoardCR?page=<%= 1 %>">First</a>
								<div>First</div></li>
							<li><a href="tdmDtMaskDashBoardCR?page=<%= currentPage-1 %>">&lt;
									Prev</a>
								<div>&lt; Prev</div> <%
			   								} else {
			   								 	if(noOfPages > 1) {
			   							%>
							<li class="disable"><a
								href="tdmDtMaskDashBoardCR?page=<%= 1 %>">First</a>
								<div>First</div></li>
							<li class="disable"><a
								href="tdmDtMaskDashBoardCR?page=<%= currentPage-1 %>">&lt;
									Prev</a>
								<div>&lt; Prev</div> <%
			   								 	}
			   								}
							...
							<%
			    						...
			    						%>
							<li><a href="tdmDtMaskDashBoardCR?page=<%= i %>"
								id="employeeLink"></a>
								<div><%= i %></div></li>
							<%
			    									}
										...
										%>
							<li><a href="tdmDtMaskDashBoardCR?page=<%= currentPage+1 %>">Next
									&gt;</a>
								<div>Next &gt;</div></li>
							<li><a href="tdmDtMaskDashBoardCR?page=<%= noOfPages %>">Last</a>
								<div>Last</div></li>
							<%
											} else {
											    if(noOfPages > 1) {
										%>
							<li class="disable"><a
								href="tdmDtMaskDashBoardCR?page=<%= currentPage+1 %>">Next
									&gt;</a>
								<div>Next &gt;</div></li>
							<li class="disable"><a
								href="tdmDtMaskDashBoardCR?page=<%= noOfPages %>">Last</a>
								<div>Last</div></li>
							<%
											    }
											}
...
window.location.hash = "myid";
   ...
   {
	   confirmation_dialogYNCancelMR(reqId,'Submit','Are you sure you want to cancel the request ?',400,'YesNo','Yes','No');
   }
