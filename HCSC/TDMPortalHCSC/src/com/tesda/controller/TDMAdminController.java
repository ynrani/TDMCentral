...
/**
 * Controller class which is used to get the details of TDMPortal logged in
 * user.
 * Handles create new user, delete existing user and also can modify the
 * user in TDMPortal.
 */
@Controller
public class TDMAdminController
{
			...
			HttpServletRequest request, HttpServletResponse response)
	{
		...
		try
		{
			...
			totalRecords = tDMAdminService.searchUserRecordsCount();
			List<TDMLoginUsersDTO> displayUsers = tDMAdminService.getAllUsers(offSet,
					recordsperpage);
			...
			model.addAttribute("displayUser", displayUsers);
			logger.info(MessageConstants.ADMIN_CNTRL + MessageConstants.DISPLYUSR
					+ MessageConstants.LOG_INFO_RETURN);
			return TDMConstants.DISPLAY_USERS_NEW;
		}
		catch (ServiceException se)
		{
			if (se.getErrorCode() != null)
	 ...
	 * @return
	 */
	@RequestMapping(value = TDMConstants.MAP_DELETEUSER, method = RequestMethod.POST)
	@ResponseBody
	...
	public @ResponseBody
	ValidationResponse validateUserId(
