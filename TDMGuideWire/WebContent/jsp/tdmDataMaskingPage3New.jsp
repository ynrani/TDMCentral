	    ...
	    <li><a class="hrefVisited" href="./tdmDtMaskDashboard">Data Masking Dashboard</a></li> -->
			...
			<div class="two-col">
				<form:form id="dataMaskingForm3" name="dataMaskingForm3"
					action="${pageContext.request.contextPath}/tdmDataMaskingPage3New"
					...
					<div>
						<table
							...
							<tr>
								...
								<th class="lable-title" width="30%" style="font-size: 14px;">Application
									Name<span>*</span>
								</th>
								<th class="lable-title" width="30%" style="font-size: 14px;">DataBase<span>*</span></th>
								<th class="lable-title" width="30%" style="font-size: 14px;">No
									of Tables<span>*</span>
								</th>
							</tr>
							<tbody class="tbodyContainer">
								<input type="hidden" name="tdgDataMaskingNoOfAppsDTOs_hidden"
									id="tdgDataMaskingNoOfAppsDTOs_hidden"
									value="${fn:length(tdgDataMaskingDTO.tdgDataMaskingNoOfAppsDTOs)-1}" />
								<c:forEach
									items="${tdgDataMaskingDTO.tdgDataMaskingNoOfAppsDTOs}"
									varStatus="status" var="tdgDataMaskingNoOfAppsDTOs">
									<c:set var="frRow" value="${status.index+1}" />
									<c:set var="listSize"
										value="${fn:length(tdgDataMaskingDTO.tdgDataMaskingNoOfAppsDTOs)}" />
									<tr id='${tdgDataMaskingNoOfAppsDTOs.id}'>
										<c:choose>
											<c:when test="${(readOnly eq null) && (disabled eq null)}">
												<td class="noWrapTxt algnRght"><c:choose>
														<c:when test="${frRow==listSize}">
															<a href="javascript:;"
																class="sprite icon-add  addRow_DMASK"
																id="tdgDataMaskingNoOfAppsDTOs">Add</a>
														</c:when>
														<c:otherwise>
															<a href="javascript:;"
																class="delRowFromDB_DMASK sprite icon-delete"
																id="tdgDataMaskingNoOfAppsDTOs">Delete</a>
														</c:otherwise>
													</c:choose></td>
											</c:when>
										</c:choose>
										<form:hidden
											path="tdgDataMaskingNoOfAppsDTOs[${status.index}].id"
											id="tdgDataMaskingNoOfAppsDTOs[${status.index}].id" />
										<td><form:input
												path="tdgDataMaskingNoOfAppsDTOs[${status.index}].appName"
												id="tdgDataMaskingNoOfAppsDTOs[${status.index}].appName"
												class="form-control listValidator mandateField" /></td>
										<td><form:input
												path="tdgDataMaskingNoOfAppsDTOs[${status.index}].dbName"
												id="tdgDataMaskingNoOfAppsDTOs[${status.index}].dbName"
												class="form-control listValidator mandateField" /></td>
										<td><form:input
												path="tdgDataMaskingNoOfAppsDTOs[${status.index}].noOfTables"
												id="tdgDataMaskingNoOfAppsDTOs[${status.index}].noOfTables"
												class="form-control listValidator mandateField" /></td>
									</tr>
								</c:forEach>
							</tbody>
