					...
					<table
						...
						<tbody class="tbodyContainer">
							<input type="hidden" name="tdgDataMaskingNoOfAppsDTOs_hidden"
								...
								value="${fn:length(tdmOnboardReqDTO.tdgDataMaskingNoOfAppsDTOs)-1}" />
							<c:forEach items="${tdmOnboardReqDTO.tdgDataMaskingNoOfAppsDTOs}"
								varStatus="status" var="tdgDataMaskingNoOfAppsDTOs">
								<c:set var="frRow" value="${status.index+1}" />
								<c:set var="listSize"
									value="${fn:length(tdmOnboardReqDTO.tdgDataMaskingNoOfAppsDTOs)}" />
								<tr id='${tdgDataMaskingNoOfAppsDTOs.id}'>
									<td class="noWrapTxt algnRght"><c:choose>
											<c:when test="${frRow==listSize}">
												<a href="javascript:;"
													class="sprite icon-add  addRow_ON_BOARD"
													id="tdgDataMaskingNoOfAppsDTOs">Add</a>
											</c:when>
											<c:otherwise>
												<a href="javascript:;"
													class="delRowFromDB_ON_BOARD sprite icon-delete"
													id="tdgDataMaskingNoOfAppsDTOs">Delete</a>
											</c:otherwise>
										</c:choose> <form:hidden
