...
<body>
	<div id="main" class="wrapper mainAll">
		...
		<div id="tabs-1" class="container">
			...
			<form:form id="reservationSubscDataForm"
				name="reservationSubscDataForm"
				action="${pageContext.request.contextPath}/myReservationExportSubc">
				<c:choose>
					<c:when test="${tDMSubscSearchResultListDTOs ne null}">
							...
							<table class="table outputtable"
								...
								<tbody>
									<c:forEach items="${tDMSubscSearchResultListDTOs}"
										...
										<tr>
											<td align="center"><a
												href="${pageContext.request.servletContext.contextPath}/unreserveSubc?id=${tDMSubscSearchResultListDTOs.subscriberId}"><spring:message
						...
						<ul class="grdPagination">
							<%
											...
											if (currentPage != 1) {
			   							%>
							<li><a href="myReservationSubc?page=<%= 1 %>">First</a>
							<div>First</div></li>
							<li><a href="myReservationSubc?page=<%= currentPage-1 %>">&lt;
									Prev</a>
							<div>&lt; Prev</div> <%
			   								} else {
			   								 	if(noOfPages > 1) {
			   							%>
							<li class="disable"><a
								href="myReservationSubc?page=<%= 1 %>">First</a>
							<div>First</div></li>
							<li class="disable"><a
								href="myReservationSubc?page=<%= currentPage-1 %>">&lt; Prev</a>
							<div>&lt; Prev</div> <%
			   								 	}
			   								}
											if(noOfPages > 1) {
							...
							<%
			    						...
			    						%>
							<li><a href="myReservationSubc?page=<%= i %>"
								id="employeeLink"><%= i %></a>
							<div><%= i %></div></li>
							<%
			    									}
										...
										%>
							<li><a href="myReservationSubc?page=<%= currentPage+1 %>">Next
									&gt;</a>
							<div>Next &gt;</div></li>
							<li><a href="myReservationSubc?page=<%= noOfPages %>">Last</a>
							<div>Last</div></li>
							<%
											} else {
											    if(noOfPages > 1) {
										%>
							<li class="disable"><a
								href="myReservationSubc?page=<%= currentPage+1 %>">Next &gt;</a>
							<div>Next &gt;</div></li>
							<li class="disable"><a
								href="myReservationSubc?page=<%= noOfPages %>">Last</a>
							<div>Last</div></li>
							<%
											    }
											}
