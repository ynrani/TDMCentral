					...
					<c:when test="${tdmPolicyPropertySearchResultDTOList ne null}">
							...
							<table class="table outputtable tablesorter" style="width: 100%;">
								<thead>
									<tr>
										...
										<th bgcolor="#E3EFFB" scope="col" class="whitefont"><spring:message
												code="label.policy.docTypeAdd" /></th>
										<th bgcolor="#E3EFFB" scope="col" class="whitefont"><spring:message
												code="label.tcId" /></th>
										<th bgcolor="#E3EFFB" scope="col" class="whitefont"><spring:message
						...
						<ul class="grdPagination">
							<%
											...
											if (currentPage != 1) {
			   							%>
							<li><a href="myReservationProp?page=<%= 1 %>">First</a>
							<div>First</div></li>
							<li><a href="myReservationProp?page=<%= currentPage-1 %>">&lt;
									Prev</a>
							<div>&lt; Prev</div> <%
			   								} else {
			   								 	if(noOfPages > 1) {
			   							%>
							<li class="disable"><a
								href="myReservationProp?page=<%= 1 %>">First</a>
							<div>First</div></li>
							<li class="disable"><a
								href="myReservationProp?page=<%= currentPage-1 %>">&lt; Prev</a>
							<div>&lt; Prev</div> <%
			   								 	}
			   								}
											if(noOfPages > 1) {
							...
							<%
			    						...
			    						%>
							<li><a href="myReservationProp?page=<%= i %>"
								id="employeeLink"><%= i %></a>
							<div><%= i %></div></li>
							<%
			    									}
										...
										%>
							<li><a href="myReservationProp?page=<%= currentPage+1 %>">Next
									&gt;</a>
							<div>Next &gt;</div></li>
							<li><a href="myReservationProp?page=<%= noOfPages %>">Last</a>
							<div>Last</div></li>
							<%
											} else {
											    if(noOfPages > 1) {
										%>
							<li class="disable"><a
								href="myReservationProp?page=<%= currentPage+1 %>">Next &gt;</a>
							<div>Next &gt;</div></li>
							<li class="disable"><a
								href="myReservationProp?page=<%= noOfPages %>">Last</a>
							<div>Last</div></li>
							<%
											    }
											}
