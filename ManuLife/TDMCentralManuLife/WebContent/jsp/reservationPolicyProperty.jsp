...
<%--             <td align="center"><a class="hrefVisited" href="javascript:;" onclick="popupuser('./reservationPolicyPropertyPopUp?id=${tdmPolicyPropertySearchResultDTOList.policynumber}','Reserved Data','popup','popupOverlay','750');">${tdmPolicyPropertySearchResultDTOList.policynumber}</a></td> --%>
        ...
        <!-- Pagination Starts -->
    								<ul class="grdPagination">
			                  			<%
											...
											if (currentPage != 1) {
			   							%>
			   									<li><a href="myReservationProp?page=<%= 1 %>"><spring:message code="label.first"/></a><div><spring:message code="label.first"/></div></li>
			   									<li><a href="myReservationProp?page=<%= currentPage-1 %>">&lt; <spring:message code="label.prev"/></a><div>&lt; <spring:message code="label.prev"/></div>
			   							<%
			   								} else {
			   								 	if(noOfPages > 1) {
			   							%>
			   								 	<li class="disable"><a href="myReservationProp?page=<%= 1 %>"><spring:message code="label.first"/></a><div><spring:message code="label.first"/></div></li>
			   									<li class="disable"><a href="myReservationProp?page=<%= currentPage-1 %>">&lt; <spring:message code="label.prev"/></a><div>&lt; <spring:message code="label.prev"/></div>
			   							<%
			   								 	}
			   								}
											if(noOfPages > 1) {
										...
										%>
			    		     					<li><a href="myReservationProp?page=<%= currentPage+1 %>"><spring:message code="label.next"/> &gt;</a><div><spring:message code="label.next"/> &gt;</div></li>
			    		     					<li><a href="myReservationProp?page=<%= noOfPages %>"><spring:message code="label.last"/></a><div><spring:message code="label.last"/></div></li>
			    		   				<%
											} else {
											    if(noOfPages > 1) {
										%>
			    		     					<li class="disable"><a href="myReservationProp?page=<%= currentPage+1 %>"><spring:message code="label.next"/> &gt;</a><div><spring:message code="label.next"/> &gt;</div></li>
			    		     					<li class="disable"><a href="myReservationProp?page=<%= noOfPages %>"><spring:message code="label.last"/></a><div><spring:message code="label.last"/></div></li>
										<%
											    }
											}
										%>
      ...
      <c:otherwise>
        <h3 style="float: left; width: 40%; border: 0; font-size: 14px;color: #E74949; padding-top: 15px">
				<u><spring:message code="label.rs.policyproperty"/></u>
			</h3>
        <br />
        <table style="width:100%; border:0">
              <tbody>
