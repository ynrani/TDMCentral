...
import com.tdm.constant.AppLocaleConstant;
...
public class TdmPolicyAutoPropSearchMapperImpl implements TdmPolicyAutoPropSearchMapper
{
	...
	public List<TdmPolicyPropertySearchResultDTO> converPolicysummaryToTdmPolicyPropertySearchDTONew(
		...
		if (policySummaryStgDOs != null && !policySummaryStgDOs.isEmpty()) {
			...
			if(!tdmPolicyPropertySearchDTO.getLocale().equalsIgnoreCase("en"))
				props= AppLocaleConstant.getProperties(tdmPolicyPropertySearchDTO.getLocale());
			for (PolicySummaryStg policySummaryStgDO : policySummaryStgDOs) {
				...
				if (StringUtils.isNotEmpty(policySummaryStgDO.getPolicystatuscd())) {
					if ( tdmPolicyPropertySearchDTO.getLocale().equalsIgnoreCase("en")) {
						...
						if ((policySummaryStgDO.getPolicystatuscd().equalsIgnoreCase(props.getProperty("appconstant.issued")) && (policySummaryStgDO
								.getTimedpolicystatuscd().equalsIgnoreCase(props.getProperty("appconstant.inForce")) || policySummaryStgDO
								.getTimedpolicystatuscd().equalsIgnoreCase(props.getProperty("appconstant.inForcePending"))))
								|| policySummaryStgDO.getPolicystatuscd().equalsIgnoreCase(props.getProperty("appconstant.active"))) {
							tdmPolicyPropertySearchResultDTO.setPolicyStage(props.getProperty("appconstant.active"));
						} else if (policySummaryStgDO.getPolicystatuscd().equalsIgnoreCase(props.getProperty("appconstant.Cancelled"))) {
							tdmPolicyPropertySearchResultDTO.setPolicyStage(props.getProperty("appconstant.Cancelled"));
						} else {
							tdmPolicyPropertySearchResultDTO.setPolicyStage(props.getProperty("appconstant.Pending"));
						}
					}
				}
				...
				if (StringUtils.isNotEmpty(policySummaryStgDO.getContracttermtypecd())) {
					if(tdmPolicyPropertySearchDTO.getLocale().equalsIgnoreCase("en")){
						if (policySummaryStgDO.getContracttermtypecd().equalsIgnoreCase("AN")) {
							...
							if (policySummaryStgDO.getContracttermtypecd().equalsIgnoreCase("AN")) {
								tdmPolicyPropertySearchResultDTO.setPolicyTerm(props.getProperty("label.autop.policyterm1"));
							} else if (policySummaryStgDO.getContracttermtypecd().equalsIgnoreCase("SA")) {
								tdmPolicyPropertySearchResultDTO.setPolicyTerm(props.getProperty("label.autop.policyterm2"));
							} else {
								tdmPolicyPropertySearchResultDTO.setPolicyTerm(policySummaryStgDO
										.getContracttermtypecd());
							}
				...
				if (StringUtils.isNotEmpty(policySummaryStgDO.getDocYn())) {
					if ("Y".equalsIgnoreCase(policySummaryStgDO.getDocYn())) {
						if(tdmPolicyPropertySearchDTO.getLocale().equalsIgnoreCase("en"))
							...
							tdmPolicyPropertySearchResultDTO.setAssoDocType(props.getProperty("label.yes"));
					} else {
						if(tdmPolicyPropertySearchDTO.getLocale().equalsIgnoreCase("en"))
							tdmPolicyPropertySearchResultDTO.setAssoDocType("No");
						else
							tdmPolicyPropertySearchResultDTO.setAssoDocType(props.getProperty("label.no"));
					}
	...
	public List<TdmPolicyAutoSearchResultDTO> converPolicysummaryToTdmPolicyAutoSearchResultDTONew(
		...
		if (policySummaryStgDOs != null && !policySummaryStgDOs.isEmpty()) {
			Properties props=null;
			if(!tdmPolicyAutoSearchDTO.getLocale().equalsIgnoreCase("en"))
				props= AppLocaleConstant.getProperties(tdmPolicyAutoSearchDTO.getLocale());
			...
			for (PolicySummaryStg policySummaryStgDO : policySummaryStgDOs) {
				...
				if (StringUtils.isNotEmpty(policySummaryStgDO.getPolicystatuscd())) {
					if ( tdmPolicyAutoSearchDTO.getLocale().equalsIgnoreCase("en")) {
						...
						if ((policySummaryStgDO.getPolicystatuscd().equalsIgnoreCase(props.getProperty("appconstant.issued")) && (policySummaryStgDO
								...
								.getTimedpolicystatuscd().equalsIgnoreCase(props.getProperty("appconstant.inForcePending"))))
								|| policySummaryStgDO.getPolicystatuscd().equalsIgnoreCase(props.getProperty("appconstant.active"))) {
							tdmPolicyAutoSearchResultDTO.setPolicyStage(props.getProperty("appconstant.active"));
						} else if (policySummaryStgDO.getPolicystatuscd().equalsIgnoreCase(props.getProperty("appconstant.Cancelled"))) {
							tdmPolicyAutoSearchResultDTO.setPolicyStage(props.getProperty("appconstant.Cancelled"));
						} else {
							tdmPolicyAutoSearchResultDTO.setPolicyStage(props.getProperty("appconstant.Pending"));
						}
					}
				}
				// Risk State
				...
				if (StringUtils.isNotEmpty(policySummaryStgDO.getContracttermtypecd())) {
					...
					if (policySummaryStgDO.getContracttermtypecd().equalsIgnoreCase("AN")) {
						...
						if (policySummaryStgDO.getContracttermtypecd().equalsIgnoreCase("AN")) {
							tdmPolicyAutoSearchResultDTO.setPolicyTerm(props.getProperty("label.autop.policyterm1"));
						} else if (policySummaryStgDO.getContracttermtypecd().equalsIgnoreCase("SA")) {
							tdmPolicyAutoSearchResultDTO.setPolicyTerm(props.getProperty("label.autop.policyterm2"));
						} else {
							tdmPolicyAutoSearchResultDTO.setPolicyTerm(policySummaryStgDO
									.getContracttermtypecd());
						}
					...
					if (StringUtils.isNotEmpty(policySummaryStgDO.getDocYn())) {
						if ("Y".equalsIgnoreCase(policySummaryStgDO.getDocYn())) {
							...
							tdmPolicyAutoSearchResultDTO.setAssoDocReq("Yes");
								...
								tdmPolicyAutoSearchResultDTO.setAssoDocReq(props.getProperty("label.yes"));
						} else {
							if(tdmPolicyAutoSearchDTO.getLocale().equalsIgnoreCase("en"))
							tdmPolicyAutoSearchResultDTO.setAssoDocReq("No");
							else
								tdmPolicyAutoSearchResultDTO.setAssoDocReq(props.getProperty("label.no"));
						}
					}
