...
public class TdmPolicySearchController
{
	...
	public String findTestDataBack(
		...
		try {
			...
			if (null != tdmPolicySearchDTO1) {
				...
				model.addAttribute(AppConstant.RESULT, getSearchCriteria(tdmPolicySearchDTO,request));
	 ...
	 * @return
	 */
	public String getSearchCriteria(TdmPolicySearchDTO tdmPolicySearchDTO,HttpServletRequest request) {
		...
		if (null != tdmPolicySearchDTO) {
			...
			if (StringUtils.isNotEmpty(tdmPolicySearchDTO.getCompany())) {
				searchCriteria += " + " + messageSource.getMessage("label.policy.comp", null, locale)
