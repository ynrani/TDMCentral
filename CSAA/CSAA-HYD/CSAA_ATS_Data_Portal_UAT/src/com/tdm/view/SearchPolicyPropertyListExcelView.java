...
public class SearchPolicyPropertyListExcelView extends AbstractExcelView {
	...
	protected void buildExcelDocument(Map<String, Object> model,
		...
		List<TdmPolicyPropertySearchResultDTO> searchPolicyAutoOutputList = (List<TdmPolicyPropertySearchResultDTO>) model
	...
	public void setExcelHeader(HSSFSheet excelSheet, HSSFCellStyle style) {
		...
		excelHeader.createCell(4).setCellValue("Effective date");
	...
	public void setExcelRows(HSSFSheet excelSheet,
			List<TdmPolicyPropertySearchResultDTO> searchPolicyAutoOutputList,
			HSSFCellStyle style) {
		int record = 5;
		if (searchPolicyAutoOutputList == null
				|| searchPolicyAutoOutputList.isEmpty()) {
			return;
		}
		for (TdmPolicyPropertySearchResultDTO tdmPolicyAutoSearchResultDTO : searchPolicyAutoOutputList) {
			HSSFRow excelRow = excelSheet.createRow(record++);
			excelRow.createCell(0).setCellValue(
					tdmPolicyAutoSearchResultDTO.getUserId());
			excelRow.createCell(1).setCellValue(
					tdmPolicyAutoSearchResultDTO.getPolicynumber());
			excelRow.createCell(2).setCellValue(
					tdmPolicyAutoSearchResultDTO.getPolicyStage());
			excelRow.createCell(3).setCellValue(
					tdmPolicyAutoSearchResultDTO.getPolicyState());
			/*excelRow.createCell(4).setCellValue(
					tdmPolicyAutoSearchResultDTO.getPolicyTerm());*/
			excelRow.createCell(4).setCellValue(
					tdmPolicyAutoSearchResultDTO.getPolicyEffectDt());

			 excelRow.createCell(5).setCellValue((tdmPolicyAutoSearchResultDTO.getTotalDue()==null?0D:Double.valueOf(tdmPolicyAutoSearchResultDTO.getTotalDue())));
