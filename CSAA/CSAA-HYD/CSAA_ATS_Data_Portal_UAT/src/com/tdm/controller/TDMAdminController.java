...
public class TDMAdminController
{
	...
	public String userDetails(@ModelAttribute(MessageConstant.USER_DO) TdmUserDTO userdo,
		...
		try {
			...
			boolean strMessage = tDMAdminService.saveUserDetails(userdo, bEdit);
	...
	public String getRequestByUserId(@ModelAttribute(MessageConstant.USER_DO) TDMDataCreationDTO userdo,
//			@RequestParam(value = AppConstant.USER_ID, required = false) String reqBy,
			@RequestParam(value = AppConstant.PAGE, required = false) String search,
			ModelMap model, HttpServletRequest request, HttpServletResponse response)
			throws BaseException {
		...
		try {
			...
			//List<TDMDataCreationDTO> reqListByUserId = tDMAdminService.getRequestByUserId(userId);
			List<TDMDataCreationDTO> reqListByUserId = tDMAdminService.getRequestByUserId(userId,userRole, offSet,recordsperpage, true);
			...
			model.addAttribute(AppConstant.TDM_ADMIN_DISPLAY_USER, reqListByUserId);
			logger.info(MessageConstant.TDM_ADMIN_CTRL + "find request"
					+ MessageConstant.LOG_INFO_RETURN);
			activityAuditService.auditActivityLog(ActivityScreenName.MY_REQUESTS, ActivityAuditLog.MY_REQUEST_TAB, userId, null);
			return "myRequests";
		} catch (BaseException baseEx) {
	...
	public String getAllRequest(//@ModelAttribute(MessageConstant.USER_DO) TdmUserDTO userdo,
//			@RequestParam(value = AppConstant.USER_ID, required = false) String reqBy,
			@RequestParam(value = AppConstant.PAGE, required = false) String search,
			ModelMap model, HttpServletRequest request, HttpServletResponse response)
			throws BaseException {
		...
		try {
			...
			List<TDMDataCreationDTO> dashBoardRequestList = tDMAdminService.getAllRequest(offSet,recordsperpage, true);
			logger.info(MessageConstant.TDM_ADMIN_CTRL + "find request"
					+ MessageConstant.LOG_INFO_RETURN);
			//totalRecords = Integer.valueOf(dashBoardRequestList.size()).longValue();
		/*	if (null != dashBoardRequestList) {
				if (dashBoardRequestList.size() >= 25) {
					totalRecords = 50L;
				} else {
					totalRecords = 25L;
				}
			...
			activityAuditService.auditActivityLog(ActivityScreenName.SERVICE_DESK, ActivityAuditLog.SERVICE_DESK_TAB, userId, null);
