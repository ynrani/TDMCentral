...
public class EmailNotificationServiceImpl implements EmailNotificationService
{
	...
	@Override
	public void sendEmail(final DCAutoEmailDTO autoEmailDTO)
			throws ServiceException {
		logger.info(MessageConstant.TDM_EMAIL_SERVICE + MessageConstant.TDM_EMAIL_SEND
				+ MessageConstant.LOG_INFO_PARAMS_NO);
		try {
			MimeMessagePreparator preparator = new MimeMessagePreparator()
			{
				...
				public void prepare(MimeMessage mimeMessage) throws Exception {
					...
					message.setFrom(autoEmailDTO.getFrom());
					String[] ccs = {"Deepak.X.Srivastava@csaa.com","Panchal.Barua@csaa.com","Arunkumar.Nagaiyasamy@csaa.com"};
					if(!ArrayUtils.isEmpty(ccs)){
						message.setCc(ccs);
					}
					/*message.setSubject(AppConstant.SUBJECT_PAS_REQUEST_STATUS
							+ autoEmailDTO.getRequestId());*/
					message.setSubject(autoEmailDTO.getSubject());
					message.setSentDate(new Date());
					Map model = new HashMap();
					model.put(AppConstant.NEW_MSG, autoEmailDTO);
					message.setText(autoEmailDTO.getMsg(), true);
				}
			};
			logger.info(MessageConstant.TDM_EMAIL_SERVICE + MessageConstant.TDM_EMAIL_SEND
					+ MessageConstant.LOG_INFO_RETURN);
