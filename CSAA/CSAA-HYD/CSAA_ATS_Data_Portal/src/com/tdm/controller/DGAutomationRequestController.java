...
public class DGAutomationRequestController {
	...
	private TDMDataCreationDTO populateDataGenarationForm(
		...
		try {
			...
			requestData = dataRequestService.saveServiceRequestData(
					requestDataDTO, isAutomatioFlag);
	...
	public void genarateScenarioDropDownValues(ModelMap model)
		...
		JSONObject jsonObjAutoCountry = new JSONObject();
		JSONArray jsonObjAutoPaymentPlan = new JSONArray();
		JSONObject jsonObjjPropertyPolicyType = new JSONObject();
		JSONObject jsonObjjPropertyRiskState = new JSONObject();
		JSONArray jsonObjjPropertyPaymentPlan = new JSONArray();
		for (Object object : autoList) {
			FieldListDTO fieldListDTO = (FieldListDTO) object;
			jsonObjAutoCountry.put(fieldListDTO.getValueCode(),
					fieldListDTO.getListValue());
			jsonObjAutoCountry.remove("Any");
		}

		for (Object object : autoPayment) {
			FieldListDTO fieldListDTO = (FieldListDTO) object;
			if (!(fieldListDTO.getListValue()).equals("Any")
					|| !(fieldListDTO.getValueCode()).equals("Any"))
				jsonObjAutoPaymentPlan.put(fieldListDTO.getListValue());

		}

		for (Object object : propertyPolicyType) {
			FieldListDTO fieldListDTO = (FieldListDTO) object;
			jsonObjjPropertyPolicyType.put(fieldListDTO.getValueCode(),
					fieldListDTO.getListValue());
			jsonObjjPropertyPolicyType.remove("Any");
		}

		for (Object object : propertyRiskState) {
			FieldListDTO fieldListDTO = (FieldListDTO) object;
			jsonObjjPropertyRiskState.put(fieldListDTO.getValueCode(),
					fieldListDTO.getListValue());
			jsonObjjPropertyRiskState.remove("Any");
		}
		for (Object object : propertyPaymentPlan) {
			FieldListDTO fieldListDTO = (FieldListDTO) object;
			if (!(fieldListDTO.getListValue()).equals("Any")
					|| !(fieldListDTO.getValueCode()).equals("Any"))
				jsonObjjPropertyPaymentPlan.put(fieldListDTO.getListValue());
		}
	...
	public void uploadAutoExcel(MultipartHttpServletRequest request,
	    ...
	    try {
			...
			outputStream = 
	                    new FileOutputStream(new File("C:\\hello\\"+fileName));
	...
	public void GenarateExcel(HttpServletRequest request,
		...
		String requestId=request.getParameter("requestID");
		 dataRequestService.genarateExcel(requestId);
