...
public class EmailNotificationServiceImpl implements EmailNotificationService
{
	...
	@Override
	public void sendEmail(final DCAutoEmailDTO DCAutoEmailDTO)
			throws ServiceException {
		logger.info(MessageConstant.TDM_EMAIL_SERVICE + MessageConstant.TDM_EMAIL_SEND
				+ MessageConstant.LOG_INFO_PARAMS_NO);
		try {
			MimeMessagePreparator preparator = new MimeMessagePreparator()
			{
				...
				public void prepare(MimeMessage mimeMessage) throws Exception {
					MimeMessageHelper message = new MimeMessageHelper(mimeMessage);
					message.setTo(DCAutoEmailDTO.getTo());
					message.setFrom("venkata-sravana-kumar.chinthalaboina@capgemini.com");
					message.setSubject(AppConstant.SUBJECT_PAS_REQUEST_STATUS
							+ DCAutoEmailDTO.getSubject());
					message.setSentDate(new Date());
					Map model = new HashMap();
					model.put(AppConstant.NEW_MSG, DCAutoEmailDTO);
					String text = VelocityEngineUtils.mergeTemplateIntoString(velocityEngine,
							AppConstant.PROP_FORGOTPASS_VM, AppConstant.UTF_8, model);
					message.setText(text, true);
				}
