...
<script type="text/javascript" src="js/script.js"></script>
 ...
 $(document).ready(function() {
	 ...
	 $("#selectedTablesMulti").dblclick(function() {
		 //alert("changed");
	 ...
	 $("#selectedTables").dblclick(function() {
		...
		/*  $("#selectedTables option[value='"+$(this).val()+"']").remove(); */
		 ...
		 $("#selectedTablesMulti option[value='"+$(this).val()+"']").show();
		 $("#selectedTables option[value='"+$(this).val()+"']").hide();
	 });
	  ...
	  $("#add-row").click(function(){
          ...
          var values = $("#values").val();
            if(values == null || values == ''){
        		  alert("Enter possible conditions for "+name);
        		  $("#values").focus();
            	  return;
        	  }
            if(type == 'IN' ){
            ...
            var title=name+"#"+type+"#";
           // mystring.replace(/'/g , "TDM_SG");
            title =title+values;
            if(title.indexOf("'") > 0){
            	title.replace("'" , "TDM_SG");
            }
            alert(title);
          var markup = "<tr><td style='border: 1px solid #cdcdcd; padding :10px; ' align='left'><input type='checkbox' name='record' title='"+name+"'></td><td style='border: 1px solid #cdcdcd; padding :10px; ' align='left'>" + name + "</td><td style='border: 1px solid #cdcdcd;' align='left'>"+type+ "</td><td style='border: 1px solid #cdcdcd; padding :10px; ' align='left' title='"+title+"'>"+values+"</td></tr>";
...
<body>
	...
	<div class="mainAll">
		...
		<section class="bodySec">
			<div class="container">
			  <form:form id="tdmProfileForm" name="tdmProfileForm" action="${pageContext.request.servletContext.contextPath}/createProfilePage" modelAttribute="tdmProfilerDTO" >
			  <form:hidden path="finalTabs" id="finalTabs"/>
          	 	...
          	 	<h2 style="color: #0098cc ;   padding-top: 5%;">Create Profile</h2>
          	 	  <table style="width:100%; border:0; font-size: 13px; cellpadding:2;   border-spacing: 5px;  padding: 2% 0% 0% 0%; ">
          	 	   <tbody>
          	 	  	  <tr>
                		<td class="lable-title" width="20%" align="left" valign="middle">Profile Name</td>
                		<td class="flied-title" width="30%" align="left" valign="middle" colspan="3"><b>${tdmProfilerDTO.profilerName}</b>
                	 ...
                	 <tr>
                		<td class="lable-title" align="left" valign="middle">Available Tables<span>*</span></td>
                  		<td class="flied-title" align="left" valign="middle">
                  		<form:select path="listTables" id="selectedTablesMulti" class="down-control" multiple="multiple" size="15">
		<%-- <c:forEach items="${tdmProfilerDTO.listTables}" var="dbConnectionsDTOs" varStatus="status">
		<option value="${dbConnectionsDTOs}">${dbConnectionsDTOs}</option>
		</c:forEach> --%>	
		<option value="ALL">ALL</option>
		<c:choose>
		<c:when test="${tdmProfilerDTO.listSelectedTabs ne null &&  not empty tdmProfilerDTO.listSelectedTabs}">
		<c:forEach items="${tdmProfilerDTO.listTables}" var="dbConnectionsDTOs" varStatus="status">
		<c:set var="refreshSent" value="${true}"/>
		<c:forEach items="${tdmProfilerDTO.listSelectedTabs}" var="dbPassedTabs" varStatus="tabstatus">
		
		<c:if test="${fn:containsIgnoreCase(dbConnectionsDTOs,dbPassedTabs)}">
		...
		<c:set var="refreshSent" value="${false}"/>
			...
			<c:if test="${refreshSent}">
			    <option value="${dbConnectionsDTOs}">${dbConnectionsDTOs}</option>
                  		...
                  		<td  class="flied-title" align="left" valign="middle">
                  		<form:select path="selectedTables" id="selectedTables" class="down-control" multiple="multiple" size="15">
		...
		<option value="${dbConnectionsDTOs}" hidden="true">${dbConnectionsDTOs}</option>
                  		...
                  		<td  class="flied-title" align="left" valign="middle">
                  		<input type="button" class="btn-primary btn-cell" id="fetchFkValues" value="Find Relational Tables" onclick="fetchRelationalTabs()"><br><br>
                  		<form:select path="relationTabs" id="relationTabs" class="down-control" multiple="multiple" size="10">
                  		</form:select>
                  		</td>
                 	  </tr>
					...
					<tbody>
					 <tr>
					   <td colspan="2" align="center" valign="middle" class="buttonsAll22">
					  	 ...
					  	 <input type="submit" name="create" id="fetchcolumns" value="Fetch Columns">
					   </td>
					 </tr>
					</tbody>
				  </table>
         	    ...
         	    <c:if test="${tdmProfilerDTO.listColumns ne null &&  not empty tdmProfilerDTO.listColumns}">
		...
		<c:forEach items="${tdmProfilerDTO.listColumns}" var="columnName1" varStatus="status">
		<option value="${columnName1}">${columnName1}</option>
		</c:forEach>
		</select>&nbsp;
		Operator  <select id="type" class="form-control-half-dictionary-small">
		<option value="=">=</option>		  
		  <option value="<">&lt;</option>  
		  <option value=">">&gt;</option>		  
		...
		Condition <input type="text" id="values" placeholder="Please enter condition" class="form-control-half-dictionary" > &nbsp;
		<input type="button" class="btn-primary btn-cell" id="add-row" value="Add Row">
		</div>
		<!-- 
        <input type="text" id="email" placeholder="Email Address" class="form-control">
    	<input type="button" class="btn-primary btn-cell" id="add-row" value="Add Row"> -->
			<table id="fileTable" style="width:100%;font-size: 13px;border:0; cellpadding:0; cellspacing:1; margin-bottom: 20px; border-collapse: collapse;">
        <thead>
            <tr>
                <th style="border: 1px solid #cdcdcd; padding: 10px;" bgcolor="#E3EFFB" scope="col" class="whitefont" align="center">Select</th>
                <th style="border: 1px solid #cdcdcd; padding: 10px;" bgcolor="#E3EFFB" scope="col" class="whitefont" align="center">Column Name</th>
                <th style="border: 1px solid #cdcdcd; padding: 10px;" bgcolor="#E3EFFB" scope="col" class="whitefont" align="center">Operator</th>
                <th style="border: 1px solid #cdcdcd; padding: 10px;" bgcolor="#E3EFFB" scope="col" class="whitefont" align="center">Conditions</th>
            </tr>
        </thead>
        <tbody>
          <!--   <tr>
 ...
 -->        </tbody>
    ...
    <input type="button" id="createDictionary" class="btn-primary btn-cell" value="Create/Update Profile" />
    </div>
    </c:if>
         	  </form:form>
			</div>
